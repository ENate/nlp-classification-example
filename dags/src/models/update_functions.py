...
def load_current_model(model_path, file_m):
    ...
    model = models.load_model(os.getcwd()+model_path + str(file_m))
...
def update_model(**kwargs):
    """Update model for preprocessing."""
    ti = kwargs['ti']
    ...
    for file_m in os.listdir(os.getcwd()+kwargs['path_current_model']):
        if 'H5' in file_m:
            ...
            with mlflow.start_run():
                ...
                # if the updated model outperforms the current model -> move current version
                # to archive and promote the updated model
...
def data_to_archive(**kwargs):
    ...
    for file_d in os.listdir(os.getcwd()+kwargs['path_new_data']):
        if 'new_samples.p' in file_d:
            os.rename(
                os.getcwd()+kwargs['path_new_data'] + file_d, os.getcwd()+kwargs['path_used_data'] + file_d)
